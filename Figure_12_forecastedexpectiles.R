...
pdf("plots/Figure12.pdf", width = 12, height = 6)
  par(mfcol = c(2,4))
  ...
  for(station in c(16, 44, 87, 107)){
    ##### Forecasted expectile curves #####
    poo_test_data <- TweedData_test %>% dplyr::filter(location == station)
    ...
    points(poo_test_data$date %>% ymd_to_decimal(), poo_test_data$nitrate %>% log())
    
    ##### Probabilistic forecast based CI #####
    predict_dates_ci <- seq(from = as.Date("2023-01-05"), to = as.Date("2023-12-31"), by = 10)
